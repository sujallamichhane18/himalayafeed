...
export function HeroSection({ scanInput, setScanInput, handleScan, isScanning, scanResult }: any) {
  ...
  // CF-Connecting-IP back to you and you alone) as a DIM placeholder in the
  // bar — visible, but not a value: nothing to backspace, no half-typed
  // state, and the moment you type it's simply gone. Prefill, never auto-scan.
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.BASE_URL}api/whoami`)
      ...
      .catch(() => { /* offline / dev without functions: bar just shows the normal placeholder */ })
  ...
  return (
    <div ref={heroRef} className="relative overflow-hidden w-full bg-app flex justify-center">
      <motion.section
        style={{ y: contentY, opacity: contentOpacity }}
        className="relative z-10 w-full pt-28 pb-12"
      >
        <div className="relative mx-auto flex w-full max-w-3xl flex-col items-start px-6 text-left">
          ...
          >
            <div className="relative w-full flex items-center group/search">
              ...
              <input
                ...
                placeholder={hintIp ? hintIp : 'Enter IP, domain, URL, or hash…'}
                className={`hero-scan-input relative h-14 md:h-16 w-full rounded-full border bg-slate-950/70 backdrop-blur-xl pl-12 md:pl-14 pr-28 md:pr-32 text-base text-white placeholder:text-slate-500 placeholder:select-none focus-visible:outline-none transition-all shadow-[0_8px_30px_-12px_rgba(0,0,0,0.8)] ${
                  invalid
